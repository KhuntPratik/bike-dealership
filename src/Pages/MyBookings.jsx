...
function MyBookings() {
  ...
  // ✅ Get userId from JWT or localStorage
  const storedUser = JSON.parse(localStorage.getItem("user"));
  const userId = jwtUser?.userId || storedUser?.userId;

  useEffect(() => {
    if (!userId) {
      console.error("❌ No userId found in AuthContext or localStorage!");
      setLoading(false);
      return;
    }
    ...
    const fetchBookings = async () => {
      try {
        const res = await fetch(
          `http://localhost:5275/api/Booking/user/${userId}`, // use userId
          {
            headers: {
  ...
  return (
    <div className="container my-5 mt-5">
      <h2 className="fw-bold text-dark text-center mb-5" style={{ marginTop: "100px" }}>
        📑 My Bookings
      </h2>
                ...
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <span
                    className={`badge px-3 py-2 ${
                      booking.status ? "bg-success" : "bg-warning"
                    }`}
                  >
