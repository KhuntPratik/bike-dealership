...
function Admin() {
  ...
  const fetchSummary = () => {
    fetch("http://localhost:5275/api/Bike/GetSummary/summary", { headers: { ...authHeaders } })
      .then((res) => res.json())
  ...
  const handleDelete = (bike) => {
    ...
    fetch(`http://localhost:5275/api/Bike/BikeDelete/${bike.bikeId}`, {
      method: "DELETE",
      headers: { ...authHeaders }
    })
      .then((res) => {
        ...
        setBike((prev) => prev.filter((b) => b.bikeId !== bike.bikeId));
        setAlertSeverity("success");
        setAlertMessage(`✅ ${bike.bikeName} deleted successfully!`);
        setAlertOpen(true);
      })
      .catch((err) => {
        console.error("Delete error:", err);
        setAlertSeverity("error");
        setAlertMessage(`❌ Error deleting bike: ${err.message}`);
        setAlertOpen(true);
      });
  };
